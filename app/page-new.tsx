              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <div className="flex flex-col">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full font-semibold mt-1 inline-block w-fit ${
                      (item.quantidadeDisponivel || 0) <= 0 
                        ? 'bg-red-200 text-red-800' 
                        : (item.quantidadeDisponivel || 0) <= 5 
                          ? 'bg-yellow-200 text-yellow-800' 
                          : 'bg-green-200 text-green-800'
                    }`}>
                      {(item.quantidadeDisponivel || 0) <= 0 
                        ? 'Esgotado' 
                        : `${item.quantidadeDisponivel} ${(item.quantidadeDisponivel || 0) === 1 ? 'disponível' : 'disponíveis'}`
                      }
                    </span>
                  </div>
                  {showComprarButton && (
                    <Button 
                      className="rounded-full bg-gradient-to-r from-green-400 to-emerald-400 hover:from-green-500 hover:to-emerald-500 text-white font-bold px-6 py-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={(item.quantidadeDisponivel || 0) <= 0}
                    >
                      {(item.quantidadeDisponivel || 0) <= 0 ? 'Esgotado' : 'Comprar'}
                    </Button>
                  )}
                </div>
              </div>
